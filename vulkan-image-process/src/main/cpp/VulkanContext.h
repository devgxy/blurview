...
class VulkanContext {
   public:
    ...
    [[nodiscard]] VkDevice device() const { return mDevice.handle(); }
    [[nodiscard]] VkQueue queue() const { return mQueue; }
    [[nodiscard]] VkCommandPool commandPool() const { return mCommandPool.handle(); }
    [[nodiscard]] VkDescriptorPool descriptorPool() const { return mDescriptorPool.handle(); }

    [[nodiscard]] uint32_t getWorkGroupSize() const { return mWorkGroupSize; }

    // Find a suitable memory type that matches the memoryTypeBits and the required properties.
    [[nodiscard]] std::optional<uint32_t> findMemoryType(uint32_t memoryTypeBits, VkFlags properties) const;
