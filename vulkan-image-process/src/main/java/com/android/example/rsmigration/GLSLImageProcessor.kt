...
class GLSLImageProcessor : ImageProcessor {
    ...
    private fun createOffscreenSurface(
        ...
        if (display == EGL14.EGL_NO_DISPLAY) {
            throw RuntimeException("Invalid EGLDisplay: ${EGL14.eglGetError()}")
        }

        // 检查 EGLContext 是否有效
        if (context == EGL14.EGL_NO_CONTEXT) {
            throw RuntimeException("Invalid EGLContext: ${EGL14.eglGetError()}")
        }
        ...
        // 创建 EGLSurface
        val surface = EGL14.eglCreatePbufferSurface(display, config, pbufferAttributes, 0)
        if (surface == null || surface == EGL14.EGL_NO_SURFACE) {
            val error = EGL14.eglGetError()
            throw RuntimeException("eglCreatePbufferSurface failed: 0x${Integer.toHexString(error)}")
        }

        // 绑定上下文和 surface
        val eglMadeCurrent = EGL14.eglMakeCurrent(display, surface, surface, context)
        if (!eglMadeCurrent) {
            val error = EGL14.eglGetError()
            throw RuntimeException("eglMakeCurrent failed: 0x${Integer.toHexString(error)}")
        }
        return surface
    }
